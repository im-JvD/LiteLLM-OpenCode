...
اگر `litellm doctor` (یا داشبورد) نشان دهد که کلیدها معتبرند ولی موقع چت، مدل‌های خاصی خطا می‌دهند، محتمل‌ترین علت **مسدودسازی جغرافیایی** است: شرکت‌های آمریکایی (Groq، Google، Cerebras و گاهی OpenRouter در بعضی شبکه‌ها) درخواست‌های IP ایران را در لبهٔ شبکه رد می‌کنند — حتی با کلید کاملاً معتبر:
...
1. یک VPN **سمت ویندوز** با حالت system-wide (TUN) روشن کنید — ترافیک WSL2 از ویندوز عبور می‌کند و از VPN رد می‌شود. سپس `litellm restart` و دوباره `litellm doctor` — همهٔ مدل‌ها باید OK شوند.
2. اگر نمی‌خواهید VPN دائمی باشد: مدل‌های قابل‌دسترس را نگه دارید (مثلاً Mistral + OpenRouter) و بقیه را از کانفیگ حذف کنید تا در لیست OpenCode ظاهر نشوند:
   - مدل‌های ناموجود را از `~/.litellm/config.yaml` حذف کنید (بلوک کامل `- model_name: ...`)
   - معادلشان را هم از `models` در `opencode.json` ویندوز حذف کنید
   - `litellm restart` (یا نصاب را دوباره اجرا کنید)
