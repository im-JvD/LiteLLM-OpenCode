...
check_wsl_environment() {
  log_info "[1/8] Checking WSL2 environment..."
...
#-------------------------------------------------------------------------------
install_docker() {
  log_info "[2/8] Installing Docker Engine (docker.io from Ubuntu apt repository)..."
...
configure_docker_mirrors() {
  log_info "[3/8] Configuring Iranian Docker Hub mirrors (${DAEMON_JSON})..."
...
collect_api_keys() {
  echo
  log_info "[4/8] API keys setup (5 providers)"
...
generate_litellm_config() {
  log_info "[5/8] Generating LiteLLM configuration: ${LITELLM_CONFIG}"
  mkdir -p "$LITELLM_DIR"
...
pull_liteLLM_image() {
  log_info "[6/8] Pulling prebuilt LiteLLM image (no build step): ${LITELLM_IMAGE}"
...
start_litellm_container() {
  log_info "[7/8] Starting LiteLLM container on port ${LITELLM_PORT}..."
  remove_existing_container
  build_docker_env_args
...
configure_opencode_windows() {
  log_info "[8/8] Configuring OpenCode on the Windows side..."
...
full_install() {
  ...
  OC_FILE=""
  configure_opencode_windows                  # step 8
...
print_install_success() {
  ...
  echo "    Re-run this tool : bash \"${SCRIPT_PATH}\""
  echo
